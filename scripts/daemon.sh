...
DATA_DIR="${OPENCODE_WECHAT_DATA_DIR:-${HOME}/.config/opencode/wechat}"
PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SERVICE_NAME="wechat-opencode"
...
macos_plist_label() {
  echo "com.wechat-opencode.bridge"
}
...
PLIST
  ...
  echo "Started wechat-opencode daemon (macOS launchd)"
}
...
macos_stop() {
  ...
  rm -f "$plist_path"
  echo "Stopped wechat-opencode daemon (macOS launchd)"
}
...
[Unit]
Description=WeChat OpenCode Bridge
Documentation=https://github.com/CG-man/wechat-opencode
...
linux_start() {
  if linux_systemd_available; then
    ...
    echo "Started wechat-opencode daemon (Linux systemd)"
  else
...
linux_stop() {
  if linux_systemd_available && systemctl --user cat "${SERVICE_NAME}" &>/dev/null; then
    ...
    echo "Stopped wechat-opencode daemon (Linux systemd)"
  else
    linux_direct_stop
  fi
}
