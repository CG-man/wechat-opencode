...
import { createSessionStore, getProviderState, setProviderState, type Session } from './session.js';
...
import { queryWithProvider } from './providers/factory.js';
...
async function handleMessage(
  ...
  if (session.state === 'processing') {
    const isLocalCommand = LOCAL_COMMAND_PREFIXES.some((prefix) => userText.startsWith(prefix));
    ...
    if (userText.startsWith('/clear')) {
      ...
      await sender.sendText(fromUserId, contextToken, '⏳ 当前正在处理上一条消息，暂时无法执行这个命令，请稍后重试');
    } else if (!userText.startsWith('/')) {
...
async function sendToProvider(
  ...
  const provider = session.provider ?? config.provider ?? 'opencode';
  const providerState = getProviderState(session, provider);
  ...
  try {
    ...
    const queryOptions: QueryOptions = {
      ...
      resume: providerState.sdkSessionId,
      model: session.model,
    ...
    let result = await queryWithProvider(provider, queryOptions);
    ...
    if (result.error && queryOptions.resume) {
      ...
      setProviderState(session, provider, { sdkSessionId: undefined });
      sessionStore.save(account.accountId, session);
      const retryResult = await queryWithProvider(provider, queryOptions);
      Object.assign(result, retryResult);
    }

    // Send result back to WeChat (show generic error to user, log details internally)
    if (result.error) {
      logger.error('Provider query error', { error: result.error, provider });
      await sender.sendText(fromUserId, contextToken, `⚠️ ${provider} 处理请求时出错，请稍后重试。`);
    } else if (result.text) {
      ...
      await sender.sendText(fromUserId, contextToken, `ℹ️ ${provider} 无返回内容（可能因权限被拒而终止）`);
    }

    // Update session with new SDK session ID
    setProviderState(session, provider, {
      sdkSessionId: result.sessionId || undefined,
    });
    session.state = 'idle';
    sessionStore.save(account.accountId, session);
  } catch (err) {
    const errorMsg = err instanceof Error ? err.message : String(err);
    logger.error('Error in sendToProvider', { error: errorMsg, provider });
